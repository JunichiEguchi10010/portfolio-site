...
* PC表示を中心に完成デザインとの差分を微調整した
* ヘッダー高さ、Hero 2カラム、セクション余白、カード・フォームの見た目を調整した
* Hero / AutoScreenCaseStudy の画像を `next/image` に置換し、`next.config.ts` に外部画像ドメインを追加した
* フォーム送信処理は未実装のまま
...
* スマホ・タブレット表示のレスポンシブ調整を実施した
* セクション余白、1カラム化、CTA縦並び、横スクロール防止を中心に調整した
* フォーム送信処理は未実装のまま
